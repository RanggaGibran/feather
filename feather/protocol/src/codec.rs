...
impl MinecraftCodec {
    ...
    pub fn encode(&mut self, packet: &impl Writeable, output: &mut Vec<u8>) -> anyhow::Result<()> {
        packet.write(&mut self.staging_buf, ProtocolVersion::V1_17)?;
    ...
    fn encode_compressed(
        ...
        VarInt(packet_length as i32).write(output, ProtocolVersion::V1_17)?;
        VarInt(data_length as i32).write(output, ProtocolVersion::V1_17)?;
    ...
    fn encode_uncompressed(&mut self, output: &mut Vec<u8>) -> anyhow::Result<()> {
        ...
        VarInt(length).write(output, ProtocolVersion::V1_17)?;
        ...
        T: Readable,
    {
        ...
        let packet = if let Ok(length) = VarInt::read(&mut cursor, ProtocolVersion::V1_17) {
            ...
            if self.received_buf.len() - length_field_length >= length.0 as usize {
                ...
                if self.compression.is_some() {
                    let data_length = VarInt::read(&mut cursor, ProtocolVersion::V1_17)?;
                    if data_length.0 != 0 {
                        let mut decoder =
                ...
                let packet = T::read(&mut cursor, ProtocolVersion::V1_17)?;
